...
PG_FUNCTION_INFO_V1(schema_json);

Datum schema_json(PG_FUNCTION_ARGS) {
    const char *relname;
    ...
    if (PG_ARGISNULL(0)) {
        elog(ERROR, "schema_json: 'relname' cannot be null");
    }
    ...
    if (reloid == InvalidOid) {
        elog(ERROR, "schema_json: relation not found");
    }
...
static char *get_attr_default_expression(Oid reloid, int16 attnum) {
    ...
    if (!HeapTupleIsValid(tup)) {
        elog(ERROR, "schema_json: could not find tuple for adrelid %u, adnum %d",
             reloid, attnum);
    }
...
PG_FUNCTION_INFO_V1(export_json);

Datum export_json(PG_FUNCTION_ARGS) {
    ...
    if (SRF_IS_FIRSTCALL()) {
        if (PG_ARGISNULL(0)) {
            elog(ERROR, "export_json: 'relname' cannot be null");
        }
        ...
        if ((ret = SPI_connect()) < 0) {
            elog(ERROR, "export_json: SPI_connect returned %d", ret);
        }
        ...
        if (!state) {
            elog(ERROR, "export_json: cannot allocate SRF state");
        }
        ...
        if (!plan) {
            elog(ERROR, "export_json: SPI_prepare_cursor failed with error %d", SPI_result);
        }
    ...
    if (SPI_processed != 1) {
        elog(ERROR, "export_json: expected exactly 1 row from cursor, but got %d rows", SPI_processed);
    }
