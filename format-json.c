...
void output_format_json_init(OutputPluginCallbacks *cb) {
    elog(DEBUG1, "json_decoder: output_format_json_init");
...
static void output_json_change(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
    ...
    /* Edited out to fit MySQL Replicator
    if (primary_key) {
    ...
    */
    ...
    if (newtuple) {
        appendStringInfoString(ctx->out, ", \"new-data\": ");
        output_json_tuple(ctx->out, newtuple, RelationGetDescr(rel));
    }
    if (oldtuple) {
        appendStringInfoString(ctx->out, ", \"old-data\": ");
...
void output_json_common_header(StringInfo out, const char *cmd,
    ...
    */
    ...
    /* Added to fit MySQL Replicator - Timestamp */
    appendStringInfoString(out, ", \"timestamp\": ");
    snprintf(timestamp, 16, "%lu", time(NULL));
    appendStringInfoString(out, timestamp);
